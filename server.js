...
app.delete('/api/admin/models/:id',auth,admin,async(req,res,next)=>{const client=await pool.connect();try{await client.query('BEGIN');await client.query('UPDATE chats SET model_id=NULL WHERE model_id=$1',[req.params.id]);const q=await client.query('DELETE FROM models WHERE id=$1 RETURNING id,name',[req.params.id]);if(!q.rows[0]){await client.query('ROLLBACK');return res.status(404).json({error:'مدل پیدا نشد.'});}await client.query('COMMIT');res.json({ok:true,deleted:q.rows[0]});}catch(e){await client.query('ROLLBACK');next(e)}finally{client.release()}});
