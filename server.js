...
app.post('/api/admin/models',auth,admin,async(req,res)=>{try{const name=safeText(req.body?.name,80),modelKey=safeText(req.body?.modelKey,120),planRaw=req.body?.minPlanId;const minPlanId=planRaw===undefined||planRaw===null||String(planRaw).trim()===''?null:Number(planRaw);if(name.length<2)return res.status(400).json({error:'نام مدل باید حداقل ۲ کاراکتر باشد.'});if(!modelKey)return res.status(400).json({error:'شناسه مدل را وارد کن.'});if(minPlanId!==null&&!Number.isInteger(minPlanId))return res.status(400).json({error:'پلن انتخاب‌شده معتبر نیست.'});if(minPlanId!==null&&!(await pool.query('SELECT id FROM plans WHERE id=$1',[minPlanId])).rows[0])return res.status(400).json({error:'پلن انتخاب‌شده پیدا نشد.'});await pool.query("ALTER TABLE models ADD COLUMN IF NOT EXISTS image_data TEXT; ALTER TABLE models ADD COLUMN IF NOT EXISTS system_prompt TEXT NOT NULL DEFAULT ''");const image=modelImageData(req.body?.imageData);const q=await pool.query('INSERT INTO models(name,model_key,description,min_plan_id,image_data,system_prompt,enabled) VALUES($1,$2,$3,$4,$5,$6,true) RETURNING *',[name,modelKey,safeText(req.body?.description,300),minPlanId,image,safeText(req.body?.systemPrompt,1000000)]);res.status(201).json({model:q.rows[0]});}catch(e){console.error('Create model failed',{code:e.code,message:e.message});if(e.code==='23505')return res.status(409).json({error:'مدلی با این شناسه از قبل وجود دارد.'});if(e.message?.includes('تصویر')||e.message?.includes('ابعاد')||e.message?.includes('حجم'))return res.status(400).json({error:e.message});res.status(400).json({error:'افزودن مدل ناموفق بود.',detail:String(e.message||'خطای نامشخص').slice(0,350)});}});
