...
نسخه 1.1.1 جزئیات پاسخ سرویس مقصد را در خطا و لاگ Railway نمایش می‌دهد. اگر پیام `HTTP 404` دیدید، endpoint کامل اشتباه است؛ اگر `HTTP 401/403` بود، کلید یا روش احراز هویت سرویس را بررسی کنید؛ و اگر `HTTP 400` بود، شناسه مدل ثبت‌شده در پنل مدیریت با مدل‌های سرویس مقصد یکی نیست. پاسخ خالی همراه با `HTTP 502` نشان می‌دهد خود سرویس مقصد یا پراکسی آن درخواست Railway را رد کرده است.
